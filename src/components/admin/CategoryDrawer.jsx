...
import SizeRangePanel from './SizeRangePanel'
...
export default function CategoryDrawer({ open, onClose }) {
  ...
  const [drillCategory, setDrillCategory] = useState(null) // category being drilled into for its subcategories
  const [drillSubcategory, setDrillSubcategory] = useState(null) // subcategory being drilled into for its size range
    ...
    <>
      <div className="fixed inset-0 z-[1100] flex justify-end bg-slate-900/50" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose?.() }}>
        <div
          ...
          className="relative flex h-full w-full max-w-md flex-col overflow-y-auto border-l border-slate-200 bg-white p-5 shadow-xl"
        >
          {Boolean(drillCategory) && (
            <button
              type="button"
              onClick={onClose}
              aria-label="Close"
              className="absolute right-5 top-5 rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-600"
            >
              ✕
            </button>
          )}
          ...
          {drillCategory && !drillSubcategory && (
            <SubcategoryPanel
              ...
              onOpenSizeRange={(subcategory) => setDrillSubcategory(subcategory)}
            />
          )}
          ...
          {drillCategory && drillSubcategory && (
            <SizeRangePanel
              ...
              onBack={() => setDrillSubcategory(null)}
            />
          )}
        </div>
      </div>
